...
exports.run = async (bot, message, args) => {
  ...
  if(message.channel.type !== "text") return;
    if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("<:no:663378512417128449> Bu komudu kullanabilmek için `Üyeleri At` yetkisine sahip olmanız gerek.");
    let reason = args.slice(1).join(' ')
    if (!args[0]) return message.channel.send("<:no:663378512417128449> Yasaklamak istediğiniz kullanıcıyı etiketleyiniz.")
    ...
    if (!user) return message.channel.send(`<:no:663378512417128449> Etiketlediğin kullanıcıyı sunucuda bulamadım.`)
    let member = message.guild.member(user)
    if (!member) return message.channel.send(`:no_entry:Etiketlediğin kullanıcıyı sunucuda bulamadım.`)
    if (member.hasPermission("MANAGE_MESSAGES")) return message.channel.send(`<:no:663378512417128449> :Kendi yetkimin üstündeki kişileri uzaklaştıramam.`)
    ...
    while (!uwu) {
      ...
      if (choice !== 'evet' && choice !== 'e') {
        message.channel.send('<:no:663378512417128449> Lütfen sadece **evet (e)** veya **hayır (h)** ile cevap verin.')
      }
